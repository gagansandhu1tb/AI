...
class Grid:
    ...
    def _place_fixed_objects(self):
        ...
        # Planet Adrian - centre of the grid
        cx, cy = self.width // 2, self.height // 2
        self.adrian_pos = (cx, cy)
        for dx in range(-2, 3):
            for dy in range(-2, 3):
                self._set(cx + dx, cy + dy, CellType.ADRIAN)

        # Petrova line - diagonal band of dense Astrophage
        for i in range(self.width):
            row = (i + 10) % self.height
            cell = self.get(i, row)
